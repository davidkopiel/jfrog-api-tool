...
1.  Open `jfrog_api_tool/apis.json`.

2.  Find the `"Artifactory"` section.

3.  Add the new API block:
...
4.  Save the file. You're done. The tool will automatically add "Get Storage Info" to the menu.
...
1.  Open `jfrog_api_tool/main.py`.

2.  Create a new helper function for your wizard (e.g., `_run_my_new_wizard()`). This function should use `questionary` to ask the user for input and return a dictionary for the JSON body.
...
3.  Find the `get_user_params()` function in `main.py`.

4.  Add an `elif` statement to hook your new wizard to the `wizard_name` you defined in `apis.json`.
...
5.  Save the file. The new complex command is now ready.
